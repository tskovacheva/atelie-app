...
**Текуща версия:** v1.4.0 · `atelie-v23`
...
**Статус:** в процес (v1.4.0 завършена — schema only)
**Ефорт:** ~4-5 deploy-аеми итерации, 2-3 седмици тест между всяка
**Риск:** висок — фундаментална архитектурна промяна

### UX решения (от обратна връзка април 2026)

**List view-то на Изделия НЕ се променя.** Изделието е едно и също — окарина, ваза, чаша. Видими: име, тип, последна снимка, текущ етап.
...
- Етапите включват: формоване, decoration преди бисквит, бисквитно изпичане, decoration след бисквит, глазиране, глазурно изпичане, post-firing techniques

### UX параметри (фиксирани)

- **Workflow при create:** Вариант А — modal-ът остава както е сега. Timeline се добавя после в detail-а.
...
- **Бутон „+ добави":** в header-а на секцията „История" (горе)
- **Очакван брой events:** 2-4 средно, рядко повече. Без virtualization, без collapse.
- **Свито/разгънато:** не нужно при този мащаб

### Деploy-аеми итерации

**v1.4.0 — Schema only ✅ ЗАВЪРШЕНА**
- Добавено `events[]` поле на piece (празно по default)
- Helper-и: `ensurePieceEvents()`, `_normalizeEvent()`
- Future event shape като коментар в код
- Старите pieces автоматично получават `events: []` при load
- Backward compat verification — никаква visible промяна
- Edit preserves existing events (важно за future iterations)

**v1.4.1 — Read-only timeline (следващо)**
...
## v1.4.0 (април 2026) — Фаза Б, итерация 1: Schema only

**Контекст:** Първата от ~5 deploy-аеми итерации на Фаза Б. Без visible промени за потребителя — само инфраструктурна подготовка.

### Промени

- Добавено `events[]` поле на piece schema (празен масив по default)
- Helper `ensurePieceEvents(piece)` — нормализира events array (defensive, idempotent, no flag)
- Helper `_normalizeEvent(e)` — генерира id ако липсва, гарантира photos array
- Future event shape документиран като коментар в код
- `load()` извиква ensurePieceEvents за всички pieces (backfill)
- `savePiece()` препазва events на edit (не ги изтрива при редакция на flat fields)
- DEF_MATS глини мигрирани директно към fireGlazeTemp (от v1.3.2)

### Защо важно

Преди да напишем UI за timeline (v1.4.1), schema-та трябва да съществува и стари pieces да имат events array. Тази версия е „infrastructure only" — нищо visible.

### Acceptance criteria — verified

- App зарежда без console errors ✓
- Старите pieces получават events: [] при load ✓
- Новите pieces имат events: [] при create ✓
- Edit на piece не изтрива events ✓
- Export backup включва events ✓
- Нищо visible не се променя ✓
- Всички текущи функции работят както преди ✓
